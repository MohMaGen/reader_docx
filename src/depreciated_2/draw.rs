...
const D_LN_SP: f32 = 1.;
const D_P_B_SP: f32 = 20.;
const D_P_A_SP: f32 = 20.;
const FONT_ML: f32 = 2.;
            ...
            match node {
                crate::docx_document::DocxNode::Paragrapth {
                    ...
                    for (line_idx, line) in lines.iter().enumerate() {
                        ...
                        canvas.draw_according_to_justification(
                            properties,
                            printer,
                            line,
                            line_height,
                        ...
                        if line_idx != lines.len() - 1 {
                            printer.y += line_height
                                + properties.spacing.line.unwrap_or(D_LN_SP) * scale_ml
                                + line_height;
                        }
                    }
...
pub trait TextHelper {
    ...
    fn draw_according_to_justification<'a>(
        &mut self,
        properties: &crate::docx_document::ParagraphProperties,
        printer: Point,
        line: &Vec<Texture>,
        line_height: f32,
        scale_ml: f32,
        page_inner_width: f32,
        line_width: f32,
        line_width_sp: f32,
    ) -> Result<(), anyhow::Error>;
}
...
impl TextHelper for Canvas<Window> {
    fn draw_according_to_justification<'a>(
        ...
        Ok(match properties.justify {
            Some(crate::docx_document::Justification::Start) | None => {
                self.draw_words_from(printer, line, line_height, D_HOR_SP, scale_ml)?;
            }
            Some(crate::docx_document::Justification::Width) => {
                let hor_sp = (page_inner_width - line_width) / line.len() as f32;
                self.draw_words_from(printer, line, line_height, hor_sp, scale_ml)?;
            }
            Some(crate::docx_document::Justification::Center) => {
                ...
                self.draw_words_from(start, line, line_height, D_HOR_SP, scale_ml)?;
            }
            Some(crate::docx_document::Justification::End) => {
                let start = ((printer.x + page_inner_width) - line_width_sp, printer.y);
                self.draw_words_from(start, line, line_height, D_HOR_SP, scale_ml)?;
            }
        })
    }
